...
def train_set_size(dataset_name='3_anonymous', cost_threshold=20000):
    client = get_client()
    db = client.milptunedb
    dataset = db[dataset_name]

    r = dataset.count_documents({"bipartite": {"$exists": True}, "default_config.0.cost": {"$lt": cost_threshold}}) 

    return r


def load_batch(dataset_name='3_anonymous', n_instances=100, start_index=0, cost_threshold=20000):
    # check locally
...
if __name__ == '__main__':
    ...
    model = InstanceEmbeddor(32, 4, 16).to(device)
